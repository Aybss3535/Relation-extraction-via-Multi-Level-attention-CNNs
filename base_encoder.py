...
class BaseEncoder(nn.Module):
    ...
    def __init__(self, 
                 ...
                 mask_entity=False):
        """
        encoder use to encode text into corresponding embedding to provide semantic features.
        Args:
        ...
        self.max_length = max_length #the max length of sentence
        self.num_token = len(token2id) #the number of token in dictionaries
        self.num_position = max_length * 2 #the length of position information，range from 0 to 2*max_length-1
        self.mask_entity = mask_entity #hide the entity

        if word2vec is None:
            self.word_size = word_size
        else:
            self.word_size = word2vec.shape[-1] #The number of columns of word2vec is the dimension of word embedding
            
        self.position_size = position_size
        self.hidden_size = hidden_size
        self.input_size = word_size + position_size * 2 #The input dimension is: word_embedded_dimension + position_embedded_dimension * 2
        self.blank_padding = blank_padding

        if not '[UNK]' in self.token2id: #将[UNK]加入到字典中
            self.token2id['[UNK]'] = len(self.token2id)
            self.num_token += 1
        if not '[PAD]' in self.token2id: #[PAD]是补全字符
        ...
        if word2vec is not None:
            ...
            if self.num_token == len(word2vec) + 2:  #add [UNK] and [PAD]
                unk = torch.randn(1, self.word_size) / math.sqrt(self.word_size)
                blk = torch.zeros(1, self.word_size)
                self.word_embedding.weight.data.copy_(torch.cat([word2vec, unk, blk], 0)) # cat by dimension 0
            else:
                self.word_embedding.weight.data.copy_(word2vec)

        # Position Embedding
        self.pos1_embedding = nn.Embedding(2 * max_length, self.position_size, padding_idx=0) #Input the distance from the head entity and output the embedded position
        self.pos2_embedding = nn.Embedding(2 * max_length, self.position_size, padding_idx=0) #Input the distance from the tail entity and output the embedded position
        self.tokenizer = WordTokenizer(vocab=self.token2id, unk_token="[UNK]") #split the sentence into the token
    ...
    def tokenize(self, item):
        ...
        if 'text' in item:
            sentence = item['text']
            is_token = False
        else: #have been split into token
            sentence = item['token']
            is_token = True
        pos_head = item['h']['pos']
        pos_tail = item['t']['pos']
